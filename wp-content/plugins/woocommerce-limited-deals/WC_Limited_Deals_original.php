	...
	class WC_Limited_Deals
	{
		...
		public function check_single()
		{
			...
			if ( $this->is_enabled( $post->id ) && !$this->is_available( $post->id ) ) {
				...
				if ( $disable == 'yes' ) {
					remove_action( 'ggd_single_product_summary', 'woocommerce_template_single_add_to_cart', 30 );
					add_action( 'ggd_single_product_summary', array( 'WC_Limited_Deals', 'custom_add_to_cart' ), 30 );
				}
			}
		}
